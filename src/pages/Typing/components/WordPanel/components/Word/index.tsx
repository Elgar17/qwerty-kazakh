...
export default function WordComponent({ word, onFinish }: { word: Word; onFinish: () => void }) {
  ...
  const getCorrectLen = () => {
    return wordState.letterStates.filter((state) => state === 'correct').length
  }
  ...
  return (
    <>
      ...
      <div lang={currentLanguageCategory} className="flex flex-col items-center justify-center pb-1 pt-4">
        {word.name && <Notation notation={word.name} />}
        <div
        ...
        >
          <div
            ...
            className={`flex min-h-16 items-center ${isTextSelectable && 'select-all'} justify-center ${
              wordState.hasWrong ? style.wrong : ''
            }`}
          >
            <div
              className="min-h-20 min-w-8 border-b-2  text-gray-600 dark:text-green-700"
              style={{ fontSize: fontSizeConfig.foreignFont.toString() + 'px' }}
            >
              {wordState.displayWord.slice(0, getCorrectLen())}
            </div>
          </div>
          {/* {pronunciationIsOpen && (
